...
class DeleteEventUseCase(val eventRepository: IEventRepository, dispatcher: CoroutineDispatcher) :
    UseCase<DeleteEventUseCase.Parameters, Unit>(dispatcher) {
    override suspend fun execute(parameters: Parameters): Result<Unit> {
        eventRepository.deleteEvent(parameters.event)
        return Success(Unit)
    }
