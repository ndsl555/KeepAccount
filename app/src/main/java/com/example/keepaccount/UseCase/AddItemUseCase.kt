...
class AddItemUseCase(val itemRepository: ISpendItemRepository, dispatcher: CoroutineDispatcher) :
    UseCase<AddItemUseCase.Parameters, Unit>(dispatcher) {
    override suspend fun execute(parameters: Parameters): Result<Unit> {
        itemRepository.insertItem(parameters.item)
        return Success(Unit)
    }
